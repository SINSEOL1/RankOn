...
public sealed class LocalizationService
{
    ...
    private static readonly Dictionary<string, string[]> Strings = new()
    {
        ...
        ["방송 출력 시작 실패"] = ["Broadcast output failed to start", "配信出力の開始に失敗", "直播输出启动失败"],
        ["사용 가능한 방송 출력 포트를 찾지 못했습니다."] = ["No available broadcast output port was found.", "利用可能な配信出力ポートが見つかりません。", "未找到可用的直播输出端口。"],
        ["출력 끄기"] = ["Turn off", "出力をオフ", "关闭输出"],
        ["출력 켜기"] = ["Turn on", "出力をオン", "开启输出"],
        ["주소"] = ["Address", "アドレス", "地址"],
        ["복사"] = ["Copy", "コピー", "复制"],
        ["미리보기"] = ["Preview", "プレビュー", "预览"],
        ["권장 크기 1200 × 720 · 투명 배경"] = ["Recommended: 1200 × 720 · Transparent background", "推奨サイズ 1200 × 720 · 透明背景", "推荐尺寸 1200 × 720 · 透明背景"],
        ...
        ["3. 너비 1200, 높이 720으로 설정합니다."] = ["3. Set width to 1200 and height to 720.", "3. 幅1200、高さ720に設定します。", "3. 设置宽度 1200、高度 720。"],
        ...
        ["PC 오버레이는 작업표시줄과 Alt+Tab에 표시되지 않습니다."] = ["The PC overlay does not appear in the taskbar or Alt+Tab.", "PCオーバーレイはタスクバーやAlt+Tabには表示されません。", "PC 悬浮层不会显示在任务栏或 Alt+Tab 中。"],
        ...
        ["방송 글자/요소 크기"] = ["Broadcast text / element scale", "配信の文字/要素サイズ", "直播文字/元素大小"],
        ...
        ["표시 항목과 스타일은 함께 적용되며, 크기는 방송과 PC에서 각각 설정합니다."] = ["Visible items and style are shared; broadcast and PC sizes are set separately.", "表示項目とスタイルは共通で、配信とPCのサイズは別々に設定します。", "显示项目和样式共用，直播与 PC 大小分别设置。"],
